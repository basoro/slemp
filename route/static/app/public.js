	...
	speed = setInterval(function () {
		...
		$.post('/task/get_task_speed', '', function (h) {
			...
			for (var g = 0; g < h.task.length; g++) {
				if (h.task[g].status == "-1" || (g == 0 && h.task[g].status == "0")) {
					if (h.task[g].type != "download") {
						var c = "";
						if (h.task[g].status == "0") {
							c = "Menunggu antrean tugas...";
						} else {
							if (h.msg) {
								var f = h.msg.split("\n");
						...
						if (h.task[g].name.indexOf("scanning") != -1) {
							...
							var stateText = h.task[g].status == "0" ? "Tunggu" : "Installing";
							b = "<li><span class='titlename'>" + h.task[g].name + "</span><span class='state' style='color: green'>" + stateText + " <img src='/static/img/ing.gif'> | <a href=\"javascript:removeTask(" + h.task[g].id + ")\">cancel</a></span><div class='cmd'>" + c + "</div></li>"
						}
					} else {
						var pre = (h.msg && h.msg.pre) ? h.msg.pre : 0;
						var used = (h.msg && h.msg.used) ? toSize(h.msg.used) : "0M";
						var total = (h.msg && h.msg.total) ? toSize(h.msg.total) : "0M";
						b = "<li><div class='line-progress' style='width:" + pre + "%'></div><span class='titlename'>" + h.task[g].name + "<a style='margin-left:130px;'>" + (used + "/" + total) + "</a></span><span class='com-progress'>" + pre + "%</span><span class='state'>" + lan.bt.task_downloading + " <img src='/static/img/ing.gif'> | <a href=\"javascript:removeTask(" + h.task[g].id + ")\">" + lan.public.close + "</a></span></li>"
					}
				} else {
		...
		for (var d = 0; d < g.data.length; d++) {
			switch (g.data[d].status) {
				case "-1":
					...
					break;
				case "0":
					if (active_rendered == 0) {
						b = "<li><span class='titlename'>" + g.data[d].name + "</span><span class='state pull-right c6' style='color: green'>Tunggu <img src='/static/img/ing.gif'> | <a class='btlink' href=\"javascript:removeTask(" + g.data[d].id + ")\">Tutup</a></span><span class='opencmd'></span><pre class='cmd'>Menunggu antrean tugas...</pre></li>";
					} else {
