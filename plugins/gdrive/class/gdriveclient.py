...
class gdriveclient():
    ...
    def set_auth_url(self, url):
        token_file = self.__server_dir + '/token.json'
        if os.path.exists(token_file):
            return mw.returnJson(True, "Verifikasi berhasil")
        ...
        try:
            ...
            return mw.returnJson(False, "Otorisasi gagal saat fetch token: " + str(e))
        ...
        if not self.set_creds():
            return mw.returnJson(False, "Verifikasi gagal，Silakan ikuti halaman1 2 3 Langkah verifikasi selesai")
        return mw.returnJson(True, "Verifikasi berhasil")
    ...
    def get_token(self, get):
        ...
        try:
            ...
            return mw.returnJson(False, "Koneksi ke Google gagal")
    ...
    def build_object_name(self, data_type, file_name):
        """Bangun nama penyimpanan objek berdasarkan tipe data
        ...
        if not prefix_dict.get(data_type):
            print("data_type Kesalahan tipe!!!")
            exit(1)
    ...
    def upload_file(self, filename, data_type=None):
        """
        get.filename Nama file setelah diunggah
        get.filepath Jalur unggah file
        Dipanggil oleh tugas terjadwal panel versi baru
        getArtinyafile_name
        :param get:
        :return:
        """
