...
local function remove_waf_drop_ip()
    ...
    if not ip or not C:is_ipaddr(ip) then 
        local data = get_return_state(-1, "Kesalahan format")
        ngx.say(json.encode(data))
        ngx.exit(200)
        return true
    end
...
local function waf_cc()
    ...
    if count then
        if count > waf_limit then 
            ...
            local reason = cycle..'kumulatif melebihi dalam detik'..waf_limit..'permintaan,Blokir' .. lock_time .. 'detik'
...
-- Apakah memenuhi syarat verifikasi paksa
...
--Paksa verifikasi apakah menggunakan browser normal untuk akses situs
...
local function disable_upload_ext(ext)
    ...
    if C:is_key(site_config[server_name]['disable_upload_ext'], ext) then
        C:write_log('upload_ext', 'Daftar hitam ekstensi unggahan')
        C:return_html(config['other']['status'],other_html)
        return true
    end
    return false
