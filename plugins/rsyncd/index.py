...
def appAuthPwd(name):
    ...
    if not os.path.exists(nameDir):
        slemp.execShell("mkdir -p " + nameDir)
...
def initDReceive():
    ...
    if not os.path.exists(lock_file):
        ...
        service_path = slemp.getServerDir()
        se = slemp.readFile(systemServiceTpl)
...
def initDSend():
    ...
    if not os.path.exists(lock_file):
        lsyncd_bin = slemp.execShell('which lsyncd')[0].strip()
        if lsyncd_bin == '':
            print('lsyncd missing!')
            exit(0)

        content = slemp.readFile(systemServiceTpl)
        content = content.replace('{$SERVER_PATH}', service_path)
...
def rsyncOp(method):
    ...
    if not slemp.isAppleSystem():
        data = slemp.execShell('systemctl ' + method + ' rsyncd')
        if data[1] == '':
            return 'ok'
        return 'fail'
...
def addRec():
    ...
    slemp.writeFile(auth_path, pwd_content)
    slemp.execShell("chmod 600 " + auth_path)
...
def getRec():
    ...
    if name == "":
        ...
        tmp["name"] = ""
        tmp["comment"] = ""
        tmp["path"] = slemp.getWwwDir()
        tmp["pwd"] = slemp.getRandomString(16)
        return slemp.returnJson(True, 'OK', tmp)

    data = getRecListDataBy(name)

    content = slemp.readFile(data['secrets file'])
...
def cmdRecSecretKey():
    ...
    content = slemp.readFile(info['secrets file'])
    pwd = content.strip().split(":")
    ...
    if len(lsyncd_list) > 0:
        for x in range(len(lsyncd_list)):
            ...
            if not os.path.exists(name_dir):
                slemp.execShell("mkdir -p " + name_dir)

            cmd_exclude = name_dir + "/exclude"
            cmd_exclude_txt = ""
            for x in t['exclude']:
                cmd_exclude_txt += x + "\n"
            slemp.writeFile(cmd_exclude, cmd_exclude_txt)
            cmd_pass = name_dir + "/pass"
            slemp.writeFile(cmd_pass, t['password'])
            slemp.execShell("chmod 600 " + cmd_pass)
            ...
            slemp.writeFile(name_dir + "/cmd", cmd)
            slemp.execShell("cmod +x " + name_dir + "/cmd")
...
def lsyncdGetExclude():
    ...
    i = res[1]
    info = slist[i]
    return slemp.returnJson(True, "OK!", info['exclude'])
...
def lsyncdRemoveExclude():
    ...
    setDefaultConf(data)
    makeLsyncdConf(data)
    return slemp.returnJson(True, "OK!", exclude_list)
