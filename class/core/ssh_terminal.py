...
class ssh_terminal:
    ...
    def debug(self, msg):
        msg = "{} - {}:{} => {} \n".format(slemp.formatDate(),
                                           self.__host, self.__port, msg)
        if not slemp.isDebugMode():
            return
        slemp.writeFile(self.__debug_file, msg, 'a+')
    ...
    def setSshdConfig(self, rep=False):
        ...
        try:
            ...
            sshd_config = slemp.readFile(sshd_config_file)
            if not sshd_config:
                return False

            if rep:
                if self.__sshd_config_backup:
                    slemp.writeFile(sshd_config_file, self.__sshd_config_backup)
                    self.restartSsh()
                return True
            ...
            if is_write:
                slemp.writeFile(sshd_config_file, sshd_config)
    ...
    def connectLocalSsh(self, sid):
        ...
        try:
            ...
            if e.find('websocket error!') != -1:
                return self.returnMsg(True, 'Connection succeeded')
            if e.find('Authentication timeout') != -1:
                self.debug("authentication timed out {}".format(e))
                return self.returnMsg(False, 'Authentication timed out, please press Enter to try again! {}'.format(e))
            if e.find('Connection reset by peer') != -1:
                self.debug('The target server actively refused the connection')
                return self.returnMsg(False, 'The target server actively refused the connection')
            if e.find('Error reading SSH protocol banner') != -1:
                self.debug('Protocol header response timed out')
                return self.returnMsg(False, 'The protocol header response timed out, the network quality with the target server is too bad：' + e)
            if not e:
                self.debug('SSH protocol handshake timeout')
                return self.returnMsg(False, "The SSH protocol handshake timed out, and the network quality with the target server is too bad")
            err = slemp.getTracebackInfo()
            self.debug(err)
            return self.returnMsg(False, "Unknown mistake: {}".format(err))

        self.debug('local-ssh: The authentication is successful, and the session channel is being built')
        ssh = self.__ps.invoke_shell(
            term='xterm', width=83, height=21)
        ssh.setblocking(0)
        self.__ssh_list[sid] = ssh
        slemp.writeLog(self.__log_type, 'Successfully logged in to the SSH server [{}:{}]'.format(
            self.__host, self.__port))
        self.debug('local-ssh: Channel built')
        return self.returnMsg(True, 'Connection succeeded!')

    def connectBySocket(self, sid):
        if not self.__host:
            return self.returnMsg(False, 'Wrong link address')
        if not self.__user:
            self.__user = 'root'
        if not self.__port:
            self.__port = 22
        ...
        while num < 5:
            num += 1
            try:
                self.debug('Trying to connect {} time'.format(num))
                ...
                if num == 5:
                    self.setSshdConfig(True)
                    self.debug('Retry connection failed, {}'.format(e))
                    return self.returnMsg(False, 'Failed to connect to target server, {}:{}'.format(self.__host, self.__port))
                else:
                    time.sleep(0.2)
        ...
        try:
            ...
            if not self.__pass and not self.__pkey:
                return self.returnMsg(False, 'Password or private key cannot both be empty: {}:{}'.format(self.__host, self.__port))

            if self.__pkey != '' and self.__type != '0':
                self.debug('Authenticating private key')
                p_file = StringIO(str(self.__pkey.replace('\\n', '\n')))
                # p_file = "/tmp/t_ssh_pkey.txt"
                # slemp.writeFile(p_file, self.__pkey.replace('\\n', '\n'))
                # slemp.execShell('chmod 600 ' + p_file)
                try:
                    p_file.seek(0)
            ...
            if e.find('Authentication failed') != -1:
                ...
                return self.returnMsg(False, 'Account or password incorrect: {}'.format(e + "," + self.__user + "@" + self.__host + ":" + str(self.__port)))
            if e.find('Bad authentication type; allowed types') != -1:
                self.debug('Authentication failed {}'.format(e))
                if self.__host in ['127.0.0.1', 'localhost'] and self.__pass == 'none':
                    return self.returnMsg(False, 'Account or password incorrect: {}'.format("Authentication failed ," + self.__user + "@" + self.__host + ":" + str(self.__port)))
                return self.returnMsg(False, 'Unsupported authentication type: {}'.format(e))
            if e.find('Connection reset by peer') != -1:
                self.debug('The target server actively refused the connection')
                return self.returnMsg(False, 'The target server actively refused the connection')
            if e.find('Error reading SSH protocol banner') != -1:
        ...
        self.debug('The authentication is successful, and the session channel is being built')
    ...
    def setAttr(self, sid, info):
        ...
        if not self.__host in ['127.0.0.1', 'localhost']:
            dst_info = slemp.getServerDir() + '/webssh/host/' + self.__host + '/info.json'
    ...
    def run(self, sid, info):
        self.__sid = sid
        if not self.__sid:
            return self.wsSend('WebSocketIO is invalid')
        ...
        if sid in self.__ssh_list:
            ...
            result = self.returnMsg(True, 'Connected')
            if result['status']:
                if type(info) == str:
                    time.sleep(0.1)
