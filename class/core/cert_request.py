    ...
    def renewCertOther(self):
        ...
        for siteName in os.listdir(cert_path):
            try:
                ...
                write_log("|-[{}] renewal failed".format(siteName))
                ...
                for i in self.__config['orders'].keys():
                    ...
                    if 'retry_count' in self.__config['orders'][i]:
                        if self.__config['orders'][i]['retry_count'] >= 5:
                            writeLog('|-The domain name is skipped this time: {}, this certificate will not be renewed due to 5 consecutive renewal failures (you can try to manually renew this certificate, the number of errors will be reset after success)'.format(
